...
def apply_benign_transforms(image_filepaths, out_dir):
    print("Benign Transformation total: ", len(image_filepaths))
...
def apply_malicious_transforms(signed_image_paths, target_image_paths, out_dir):
    ...
    for target_image_path in target_image_paths:
        for signed_image_path in signed_image_paths:
            ...
            try:
                # Target Image : face image
                # Signed image: body image
                swap_faces(target_image_path, signed_image_path, output_file_name_fs, l2s)
            except:
                print("Error in shallowfakes")
                #unable_to_malicously_swap =  unable_to_malicously_swap + 1
                unable_to_malicously_swap.append(pic_b_name)
                delete_list.append(i)
                #del signed_image_paths[i]
    ...
    for val in range(len(delete_list)):
        del signed_image_paths[val]
    assert len(transformed_image_filepaths['face_swap']) >= 0

    return unable_to_malicously_swap,["face_swap"], transformed_image_filepaths, l2s, signed_image_paths

# https://github.com/guipleite/CV2-Face-Swap
def swap_faces(face_image_path, body_image_path, output_path, l2s):
    ...
    # Calculate loss?
    #print(body_image_path)
    #print(output_path)
    image_from_file = skimage.io.imread(body_image_path) / 255.0
    ...
    image_from_file = skimage.io.imread(output_path) / 255.0
    image_from_file = skimage.transform.resize(image_from_file, (300, 300),
                                               preserve_range=True, mode='constant')
    image_from_file = image_from_file[:, :, :3]
    mal_image = torch.from_numpy(image_from_file).float()

    l2_error = F.mse_loss(watermarked_image, mal_image)
    l2s.append(l2_error)
